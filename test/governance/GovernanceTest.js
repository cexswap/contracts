...
*/
  describe('Deposits - Withdraws - Swap', async function () {
    ...
    describe('Deposit after swap', async function () {
      ...
      it('Deposit should not balanced after swap', async function () {
        ...
        expect(await getReceivedTokenAmount(
          this.swap,
          wallet2,
          () => this.swap.deposit([bal.weth('2'), bal.usdt('150')], [bal.weth('2'), bal.usdt('150')], { from: wallet2 })
    ...
    /*
  describe('Swap governance vote', async function () {
    beforeEach(async function() {
      this.creator = await PoolCreator.new();
      this.factory = await SwapFactory.new(wallet1, this.creator.address, _);
      await this.factory.updateStakeChanged(wallet1, ether('150'));
      await this.factory.defaultVoteFee('0', { from: wallet1 });
    ...
    it('Be able to vote for swap fee', async function () {
      await this.swap.deposit([bal.weth('1'), bal.usdt('150')], [bal.zero, bal.zero], { from: wallet1 });
      await setBlockTime((await time.latest()).add(await this.swap.getDecayPeriod()));
      await this.swap.voteFee(bal.weth('0.0009'), { from: wallet1 })
      expect(await this.swap.getFee()).to.be.bignumber.equal('0');
      await setBlockTime((await time.latest()).addn(86500));
      expect(await this.swap.getFee()).to.be.bignumber.equal('0.0009');
    });
  });
  */
